...
export const addItem = (city, cityImage) => {
  return (dispatch, getState, url) => {
    ...
    axios.get(url,
      ...
      .then( (response) => {
        ...
        dispatch( dataResultHandler(DATA_STATUS_HANDLER, 'addSuccess', true) );
        dispatch( {type: ADD_ITEM, payload: {name: city, image: cityImage} } );
      ...
      .catch( error => {
        ...
        console.log("Error has occured in loading data...");
        console.log(error);
        dispatch( dataResultHandler(DATA_STATUS_HANDLER, 'addError', true) );
